...
export default function MentorDirectory() {
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-900 border border-slate-800 rounded-3xl p-8">
        <div>
      ...
      {/* States */}
      {loading && (
        <p className="text-slate-500 text-sm text-center py-12">Loading mentors…</p>
      )}
      {error && (
        <p className="text-amber-400 text-sm text-center py-12">Failed to load: {error}</p>
      )}
      {!loading && !error && filtered.length === 0 && (
        <p className="text-slate-500 text-sm text-center py-12">No mentors match your search.</p>
      )}
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="lg:w-80 p-8 border-b lg:border-b-0 lg:border-r border-slate-800/50 bg-slate-950/40 relative">
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <h3 className="text-xl font-black text-white tracking-tight mb-1 uppercase">{mentor.name}</h3>
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">
              {mentor.industries?.join(' · ') ?? '—'}
            </p>
            <p className="text-xs text-slate-400 italic mb-6 leading-relaxed">"{mentor.bio}"</p>

            <div className="w-full grid grid-cols-2 gap-3">
              <div className="p-3 bg-slate-950/40 rounded-xl border border-slate-800 flex flex-col items-center">
                <p className="text-[9px] text-slate-600 font-black uppercase tracking-tighter mb-1">Experience</p>
                <p className="text-sm font-black text-emerald-400 tabular-nums">{mentor.experienceYears} yrs</p>
              </div>
              <div className="p-3 bg-slate-950/40 rounded-xl border border-slate-800 flex flex-col items-center">
                <p className="text-[9px] text-slate-600 font-black uppercase tracking-tighter mb-1">Capacity</p>
        ...
        <div className="lg:w-48 p-8 flex flex-col items-center justify-center bg-indigo-500/[0.02] border-t lg:border-t-0 lg:border-l border-slate-800/50">
          ...
          <div className="relative w-24 h-24 flex items-center justify-center">
            <svg className="w-full h-full -rotate-90 drop-shadow-[0_0_8px_rgba(99,102,241,0.2)]">
              <circle cx="48" cy="48" r="40" fill="none" stroke="#0f172a" strokeWidth="8" />
              <motion.circle
            ...
            <span className="absolute text-2xl font-black text-white tabular-nums tracking-tighter">
              {masterScore}
            </span>
          </div>
          ...
          <button className="mt-4 flex items-center gap-1.5 text-[10px] font-black text-indigo-400 hover:text-white transition-colors uppercase tracking-widest bg-indigo-500/5 px-3 py-1.5 rounded-lg border border-indigo-500/10">
